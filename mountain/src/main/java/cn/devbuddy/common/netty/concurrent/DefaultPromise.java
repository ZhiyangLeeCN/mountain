...
import static cn.devbuddy.common.netty.internal.ObjectUtil.checkNotNull;
...
public class DefaultPromise<V> extends AbstractFuture<V> implements Promise<V> {
    ...
    private static final int MAX_LISTENER_STACK_DEPTH = Math.min(8,
            SystemPropertyUtil.getInt("cn.devbuddy.defaultPromise.maxListenerStackDepth", 8));
     ...
     *
     * It is preferable to use {@link TaskExecutor#newPromise()} to create a new promise
     *
     * @param executor
     *        the {@link TaskExecutor} which is used to notify the promise once it is complete.
     *        It is assumed this executor will protect against {@link StackOverflowError} exceptions.
     *        The executor may be used to avoid {@link StackOverflowError} by executing a {@link Runnable} if the stack
     *        depth exceeds a threshold.
     *
     */
    public DefaultPromise(TaskExecutor executor) {
     ...
     * @param taskExecutor the executor to use to notify the listener {@code listener}.
     * @param future the future that is complete.
     * @param listener the listener to notify.
     */
    protected static void notifyListener(
            TaskExecutor taskExecutor, final Future<?> future, final GenericFutureListener<?> listener) {
        notifyListenerWithStackOverFlowProtection(
                checkNotNull(taskExecutor, "eventExecutor"),
     ...
     */
    private static void notifyListenerWithStackOverFlowProtection(final TaskExecutor executor,
                                                                  final Future<?> future,
    ...
    private static void safeExecute(TaskExecutor executor, Runnable task) {
        try {
