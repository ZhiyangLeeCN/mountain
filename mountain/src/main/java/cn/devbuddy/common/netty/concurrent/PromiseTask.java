...
class PromiseTask<V> extends DefaultPromise<V> implements RunnableFuture<V> {
    ...
    PromiseTask(TaskExecutor executor, Runnable runnable, V result) {
        super(executor);
        task = result == null ? runnable : new RunnableAdapter<V>(runnable, result);
    }

    PromiseTask(TaskExecutor executor, Runnable runnable) {
        super(executor);
        task = runnable;
    }

    PromiseTask(TaskExecutor executor, Callable<V> callable) {
        super(executor);
        task = callable;
    }
