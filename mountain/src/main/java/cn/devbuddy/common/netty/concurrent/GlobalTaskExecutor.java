...
import cn.devbuddy.common.netty.internal.ThreadExecutorMap;
...
/**
 * Single-thread singleton {@link TaskExecutor}.  It starts the thread automatically and stops it when there is no
 * task pending in the task queue for 1 second.  Please note it is not scalable to schedule large number of tasks to
 * this executor; use a dedicated executor.
 */
public final class GlobalTaskExecutor extends AbstractScheduledTaskExecutor implements OrderedEventExecutor {

    private static final InternalLogger logger = InternalLoggerFactory.getInstance(GlobalTaskExecutor.class);

    private static final long SCHEDULE_QUIET_PERIOD_INTERVAL = TimeUnit.SECONDS.toNanos(1);

    public static final GlobalTaskExecutor INSTANCE = new GlobalTaskExecutor();
    ...
    final class TaskRunner implements Runnable {
        ...
        public void run() {
            for (;;) {
                ...
                Queue<ScheduledFutureTask<?>> scheduledTaskQueue = GlobalTaskExecutor.this.scheduledTaskQueue;
